// CROSSFIRE dashboard — live on-chain state, styled to match the design
// system from public/walkthrough/ (Bull electric blue · Bear hot red ·
// Space Grotesk display · JetBrains Mono numerics).
//
// Every number is a real read of Base Sepolia state via viem, or a snapshot
// persisted to .crossfire/state.json by the duel/relay scripts. No
// fabricated data. The link at the top opens the animated walkthrough that
    ...
    <main style={{ background: CF.bg, color: CF.text, minHeight: '100vh', padding: 28, maxWidth: 1440, margin: '0 auto' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <a href="/walkthrough/index.html" target="_blank" rel="noreferrer" style={{
            display: 'inline-flex', alignItems: 'center', gap: 8,
            padding: '10px 16px', borderRadius: 8, textDecoration: 'none',
            background: `linear-gradient(90deg, ${CF.bullDeep}, ${CF.bearDeep})`,
            border: `1px solid ${CF.edgeHi}`, color: CF.text,
            fontFamily: CF.display, fontSize: 13, fontWeight: 600, letterSpacing: 0.3,
            boxShadow: `0 0 20px color-mix(in oklab, ${CF.bull} 15%, transparent), inset 0 0 20px color-mix(in oklab, ${CF.bear} 8%, transparent)`,
          }}>
            <span style={{ color: CF.bull }}>▶</span>
            Watch the duel walkthrough
          </a>
          <a href="/" style={{
